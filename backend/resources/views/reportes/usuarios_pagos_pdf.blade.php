...
<head>
    <meta charset="UTF-8">
    <title>Reporte de Pagos por Empleado - NovaRider</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sin-pago { color: #929079; font-style: italic; }
    </style>
</head>
<body>
    <div class="page">
        <div class="header">
            ...
            <div class="header-right">
                <div class="report-title">Reporte de Últimos Pagos por Empleado</div>
        ...
        <div class="summary">
            Mostrando <strong>{{ $totalRegistros }}</strong> empleado{{ $totalRegistros !== 1 ? 's' : '' }}
        </div>
        ...
        <table>
            ...
            <tbody>
                @forelse($usuarios as $u)
                    @php
                        $rolLower = strtolower(($u['roles'][0]['nombre'] ?? ''));
                        $badgeClass = 'badge-default';
                        if ($rolLower === 'administrador') $badgeClass = 'badge-admin';
                        elseif ($rolLower === 'cajero') $badgeClass = 'badge-cajero';
                        elseif (in_array($rolLower, ['mecánico', 'mecanico'])) $badgeClass = 'badge-mecanico';
                    ...
                    <tr>
                        ...
                        <td class="col-role">
                            <span class="badge {{ $badgeClass }}">{{ $u['roles'][0]['nombre'] ?? '—' }}</span>
                        </td>
                        <td>
                            @if($u['ultimo_pago'])
                                {{ $meses[$u['ultimo_pago']['mes'] - 1] }} {{ $u['ultimo_pago']['anio'] }}
                            @else
                                <span class="sin-pago">Sin pagos</span>
                            @endif
                        </td>
                        <td class="col-num">
                            @if($u['ultimo_pago'])
                                <strong>Bs {{ number_format($u['ultimo_pago']['sueldo_neto'], 2) }}</strong>
                            @else
                                <span class="sin-pago">—</span>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="no-data">No se encontraron empleados con los filtros aplicados</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
